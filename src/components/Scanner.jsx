...
const resolveSelectedCamera = (selectedCameraId, cameras = []) => {
    if (selectedCameraId === MACRO_CAMERA_OPTION_ID) {
        ...
        return {
            mode: 'macro',
            startTarget: macroCandidate?.id || rearCandidate?.id || { facingMode: { exact: 'environment' } }
        };
    }
...
const buildMacroConstraintSteps = (capabilities = {}, settings = {}) => {
    ...
    if (focusModes.includes('continuous')) {
        steps.push({ advanced: [{ focusMode: 'continuous' }] });
    } else if (focusModes.includes('single-shot')) {
        steps.push({ advanced: [{ focusMode: 'single-shot' }] });
    }

    const zoomCapability = capabilities.zoom;
    const zoomMin = Number.isFinite(zoomCapability?.min) ? zoomCapability.min : null;
...
const Scanner = ({ onComplete }) => {
    ...
    }, [cameras, handleScannedContent, selectedCameraId, status, stopScanner]);
