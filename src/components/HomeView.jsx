...
import {
    ...
    formatCurrency,
    getStoredOpportunitySignature
} from '../utils/financeInsights';
import { analyzeFoodPurchases } from '../utils/foodAnalysis';
import { construirIndiceSaudeFinanceira } from '../utils/financialHealth';
...
const HomeView = ({
    ...
    const currentMonthReceipts = useMemo(() => {
        return receipts.filter((receipt) => getMonthKey(receipt.date) === currentMonthKey);
    }, [currentMonthKey, receipts]);
    const currentMonthReceiptIds = useMemo(
        () => new Set(currentMonthReceipts.map((receipt) => receipt.id)),
        [currentMonthReceipts]
    );
    const currentMonthProducts = useMemo(() => {
        return products.filter((product) => currentMonthReceiptIds.has(product.receiptId));
    ...
    const foodAnalysis = useMemo(
        ...
        [currentMonthProducts, currentMonthReceipts, foodClassificationOverrides, productAliases]
    );

    const financialHealthModel = useMemo(
        () => construirIndiceSaudeFinanceira({
            receipts,
            products,
            pixExpenses,
            productAliases,
        ...
        [foodClassificationOverrides, pixExpenses, productAliases, products, receipts]
    );
    ...
    const opportunitySignature = useMemo(
        () => buildOpportunitySignature(insights.currentWeekdayBestPriceProducts, {
            alerts: foodAnalysis.alerts
        }),
        [foodAnalysis.alerts, insights.currentWeekdayBestPriceProducts]
    );
    const hasUnreadOpportunity = useMemo(
        () => opportunitySignature !== getStoredOpportunitySignature(),
            ...
            <MotionSection {...sectionTransition} className="hero-panel">
                <div className="hero-topbar">
                    ...
                    <div className="hero-actions">
                        <button
                            type="button"
                            className={`icon-button ${hasUnreadOpportunity ? 'has-badge' : ''}`}
                            onClick={handleNotificationClick}
                            aria-label="Abrir oportunidade do dia"
                        >
                            <Bell size={18} />
                        </button>
                ...
                <div className="hero-grid">
                    ...
                    <div className="hero-health-card">
                        <FinancialHealthCard model={financialHealthModel} />
                    </div>
                </div>
