...
const HomeView = ({
    ...
    const variationClassName = insights.variation.direction === 'up'
        ...
        : insights.variation.direction === 'down'
            ? 'positive'
            : 'neutral';
    const currentWeekdayProducts = insights.currentWeekdayBestPriceProducts?.products || [];
    const visibleCurrentWeekdayProducts = currentWeekdayProducts.slice(0, 8);
    const remainingCurrentWeekdayProducts = Math.max(0, currentWeekdayProducts.length - visibleCurrentWeekdayProducts.length);
                        ...
                        <div className="health-ring-wrapper">
                            ...
                            <div className="hero-mini-grid">
                                <div className="mini-stat">
                                    <Wallet size={16} />
                                    <div>
                                        <span>Fixos mensais</span>
                                        <strong>{formatCompactCurrency(insights.fixedMonthlyTotal)}</strong>
                                    </div>
                                </div>
                                <div className="mini-stat">
                                    <ReceiptText size={16} />
                                    <div>
                                        <span>Variaveis</span>
                                        <strong>{formatCompactCurrency(insights.variableTotal)}</strong>
                                    </div>
                                </div>
                            </div>
                    </div>
                        ...
                        <div className="premium-surface">
                            <div className="section-heading">
                                <div>
                                    ...
                                    <h2>Fixos mensais vs variaveis</h2>
                                </div>
                            </div>
                            <div className="comparison-stack">
                                <div className="comparison-row positive">
                                    <div>
                                        <span>Fixos mensais</span>
                                        <strong>{formatCurrency(insights.fixedMonthlyTotal)}</strong>
                                    </div>
                                    <small>
                                        {insights.currentMonthTotal > 0
                                            ? `${((insights.fixedMonthlyTotal / insights.currentMonthTotal) * 100).toFixed(1)}% das saidas`
                                            : 'Sem movimentacao'}
                                    </small>
                                </div>
                                <div className="comparison-row neutral">
                                    <div>
                                        <span>Variaveis</span>
                                        <strong>{formatCurrency(insights.variableTotal)}</strong>
                                    </div>
                                    <small>
                                        {insights.currentMonthTotal > 0
                                            ? `${((insights.variableTotal / insights.currentMonthTotal) * 100).toFixed(1)}% das saidas`
                                            : 'Sem movimentacao'}
                                    </small>
                                </div>
                            </div>
                            <div className="insight-list home-section-block">
                    ...
                    <MotionSection {...sectionTransition} className="premium-surface">
                        <div className="section-heading">
                            <div>
                                ...
                                <h2>Hoje e {insights.currentWeekdayBestPriceProducts.fullLabel}</h2>
                            </div>
                            <div className="status-pill positive">
                                {insights.currentWeekdayBestPriceProducts.totalProducts} produto{insights.currentWeekdayBestPriceProducts.totalProducts === 1 ? '' : 's'}
                            </div>
                        </div>
                        <div className="comparison-stack">
                            <div className="comparison-row positive">
                                <div>
                                    <span>Menor preco por historico</span>
                                    <strong>{insights.currentWeekdayBestPriceProducts.fullLabel}</strong>
                                </div>
                                <small>
                                    Produtos que costumam atingir o menor preco observado quando comprados neste dia.
                                </small>
                            </div>
                        </div>
                        <div className="insight-list home-section-block">
                            {visibleCurrentWeekdayProducts.length > 0 ? visibleCurrentWeekdayProducts.map((product) => (
                                ...
                                <div className="inline-empty">
                                    Ainda nao ha produtos com preco comparavel para {insights.currentWeekdayBestPriceProducts.fullLabel}.
                                </div>
                            )}
                        </div>
                        <div className="hero-mini-grid home-section-block">
                            <div className="mini-stat">
                                ...
                                <div>
                                    <span>Dia atual</span>
                                    <strong>{insights.currentWeekdayBestPriceProducts.shortLabel}</strong>
                                </div>
                            </div>
                            <div className="mini-stat">
                                <ReceiptText size={16} />
                                <div>
                                    ...
                                    <strong>{insights.currentWeekdayBestPriceProducts.totalProducts}</strong>
                                </div>
                            </div>
                        </div>
                        {remainingCurrentWeekdayProducts > 0 && (
                            <div className="inline-empty" style={{ marginTop: '16px' }}>
                                Mais {remainingCurrentWeekdayProducts} produto{remainingCurrentWeekdayProducts === 1 ? '' : 's'} aparecem no detalhamento completo de relatorios.
                            </div>
                        )}
                    </MotionSection>
