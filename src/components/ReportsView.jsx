...
import { db } from '../db';
import { buildFinanceOverview, formatCompactCurrency, summarizeReceiptsByOrigin } from '../utils/financeInsights';
import {
...
const ReportsView = ({ onOpenPlanning }) => {
    ...
    const scopedReceipts = useMemo(
        () => receipts.filter((receipt) => isDateKeyWithinRange(toLocalDateKey(receipt.date), normalizedDateRange)),
        [normalizedDateRange, receipts]
    );
    const scopedReceiptIds = useMemo(
        () => new Set(scopedReceipts.map((receipt) => receipt.id)),
        [scopedReceipts]
    );
    const scopedProducts = useMemo(
        () => products.filter((product) => scopedReceiptIds.has(product.receiptId)),
        [products, scopedReceiptIds]
    );
    const scopedPixExpenses = useMemo(
        () => pixExpenses.filter((expense) => isDateKeyWithinRange(toLocalDateKey(expense.date), normalizedDateRange)),
        [normalizedDateRange, pixExpenses]
    );
    const scopedReceiptOriginSummary = useMemo(
        () => summarizeReceiptsByOrigin(scopedReceipts),
        [scopedReceipts]
    );
    const scopedTotal = useMemo(
        () => (
            scopedReceipts.reduce((sum, receipt) => sum + (Number(receipt.totalValue) || 0), 0)
            + scopedPixExpenses.reduce((sum, expense) => sum + (Number(expense.value) || 0), 0)
        ),
        [scopedPixExpenses, scopedReceipts]
    );
    const scopedTransactionsCount = scopedReceipts.length + scopedPixExpenses.length;
    const scopedReferenceDate = useMemo(
        () => normalizedDateRange.endDate || new Date(),
    ...
    const insights = useMemo(
        () => buildFinanceOverview({
            ...
            pixExpenses: scopedPixExpenses,
            productAliases,
            foodClassificationOverrides,
            referenceDate: scopedReferenceDate
        }),
        [
            foodClassificationOverrides,
            productAliases,
            ...
            <MotionSection {...sectionTransition} className="page-intro-card">
                <div className="section-heading section-heading-inline">
                    ...
                    <div className="status-pill neutral">
                        {scopedTransactionsCount} lancamentos no intervalo: {scopedReceiptOriginSummary.importedReceiptsCount} importados, {scopedReceiptOriginSummary.manualReceiptsCount} manuais e {scopedPixExpenses.length} Pix
                    </div>
                </div>
                ...
                <div className="info-strip-grid">
                    <div className="info-strip">
                        <Sparkles size={16} />
                        <span>Saidas do intervalo: {formatCompactCurrency(scopedTotal)}</span>
                    </div>
                    <div className="info-strip">
                        <BarChart3 size={16} />
                        <span>Score do recorte: {insights.healthScore.score}/100</span>
                    </div>
                    <div className="info-strip">
                        <History size={16} />
                        <span>{scopedTransactionsCount} transacoes no intervalo</span>
                    </div>
                </div>
