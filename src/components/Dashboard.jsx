...
const Dashboard = () => {
    ...
    // Usamos useMemo para derivar os dados dos gráficos. 
    // Isso evita o erro #185 (loop de atualização)
    const stats = React.useMemo(() => {
        ...
        return {
            ...
            topProducts: Object.entries(prodMap)
                ...
                .slice(0, 5),
            establishmentData: Object.entries(estMap)
                .map(([name, value]) => ({ name: String(name).substring(0, 10), value }))
                .sort((a, b) => b.value - a.value)
                .slice(0, 5),
            monthlyData: Object.entries(monthsMap).map(([name, value]) => ({ name, value }))
        };
    }, [products, receipts]);
    ...
    return (
        <div className="animate-slide-up">
            ...
            {stats.establishmentData && stats.establishmentData.length > 0 && (
                <div className="glass-card">
                    <h4 style={{ marginBottom: '15px' }}>Top 5 Lojas</h4>
                    <div style={{ height: '200px' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={stats.establishmentData}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#eee" />
