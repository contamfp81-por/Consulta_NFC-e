...
const Dashboard = ({ onOpenForecastPrecision }) => {
    ...
    const stats = useMemo(() => {
        ...
        filteredReceipts.forEach((receipt) => {
            ...
            const weekday = WEEKDAY_OPTIONS.find((item) => item.index === receiptDate.getUTCDay()) || WEEKDAY_OPTIONS[0];
        ...
        const weekdayProductInsights = WEEKDAY_OPTIONS.map((weekday) => {
            ...
            const cheapestProducts = products
                .filter((product) => product.averageUnitPrice > 0)
                .sort((left, right) => (
        ...
        const hasPurchaseDataByWeekday = heatmapData.some((item) => item.value > 0);
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        {day.cheapestProducts.length > 0 ? (
                                                ...
                                                {day.cheapestProducts.map((product) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div style={{ fontSize: '0.72rem', color: '#86EFAC', marginTop: '4px' }}>
                                                            media {formatCurrencyValue(product.averageUnitPrice)} • menor {formatCurrencyValue(product.lowestUnitPrice)}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <div style={{ fontSize: '0.82rem', color: 'var(--text-light)' }}>
                                                Ainda nao ha comparativo de preco suficiente neste dia.
                                            </div>
                                        )}
                                    </div>
                                </div>
