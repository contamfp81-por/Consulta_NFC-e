...
import {
    ...
    Treemap, ScatterChart, Scatter, ZAxis, Legend, ComposedChart, LabelList
} from 'recharts';
import { db } from '../db';
...
const Dashboard = () => {
    ...
    const stats = useMemo(() => {
        ...
        const uniqueProductsList = [...new Set(products.map(p => p.name))].sort();
        ...
        if (selectedProductName) {
            const productMatch = products
                ...
                .map(p => {
                    ...
                    return {
                        date: receipt ? new Date(receipt.date).toISOString().split('T')[0] : 'Desconhecida',
                        price: Number(p.unitPrice)
                    };
                })
                .sort((a, b) => a.date.localeCompare(b.date));
            productEvolutionData = productMatch;
        }
    ...
    }, [receipts, products, selectedProductName]);
    ...
    return (
        <div className="animate-slide-up" style={{ paddingBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }}>
                ...
                <ChartCard title="Evolução de Preço por Produto" icon={TrendingUp} fullWidth>
                    <div style={{ marginBottom: '15px' }}>
                        <div style={{ position: 'relative' }}>
                            ...
                            >
                                <option value="">Selecione um produto para analisar...</option>
                                {stats.uniqueProductsList.map(name => (
                    ...
                    {selectedProductName ? (
                        ...
                        <div style={{ height: '70%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-light)', fontSize: '0.9rem' }}>
                            Escolha um produto acima para ver o histórico de preços.
                        </div>
                    )}
                </ChartCard>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
                    <ChartCard title="Distribuição por Loja (%)" icon={PieIcon}>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
