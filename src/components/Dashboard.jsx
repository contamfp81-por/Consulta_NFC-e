...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (products.length > 0) {
            ...
            const catMap = {};
            products.forEach(p => {
                catMap[p.category] = (catMap[p.category] || 0) + p.totalValue;
            });
            ...
            const prodMap = {};
            products.forEach(p => {
                prodMap[p.name] = (prodMap[p.name] || 0) + p.totalValue;
            });
            setTopProducts(
        ...
        if (receipts.length > 0) {
            ...
            const estMap = {};
            receipts.forEach(r => {
                estMap[r.establishment] = (estMap[r.establishment] || 0) + r.totalValue;
            });
            ...
            receipts.forEach(r => {
                ...
                monthsMap[month] = (monthsMap[month] || 0) + r.totalValue;
            });
            setMonthlyData(Object.entries(monthsMap).map(([name, value]) => ({ name, value })));
        }
    }, [products, receipts]);

    const totalSpent = receipts.reduce((acc, curr) => acc + curr.totalValue, 0);
