...
const Dashboard = () => {
    ...
    const stats = useMemo(() => {
        ...
        filteredProducts.forEach((product) => {
            ...
            const productName = (product.name || 'Produto sem nome').trim();
            if (!productHistoryMap[productName]) {
                productHistoryMap[productName] = {};
            }

            if (!productHistoryMap[productName][dateKey]) {
                productHistoryMap[productName][dateKey] = {
                    date: dateKey,
                    totalValue: 0,
                    totalQuantity: 0,
            ...
            productHistoryMap[productName][dateKey].totalValue += unitPrice * quantity;
            productHistoryMap[productName][dateKey].totalQuantity += quantity;
            productHistoryMap[productName][dateKey].category = product.category || productHistoryMap[productName][dateKey].category || 'Outros';
        });

        const productInflationData = Object.entries(productHistoryMap)
            .map(([name, historyByDate]) => {
