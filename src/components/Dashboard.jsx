...
import { generateConsumptionAnalysisPdf } from '../utils/consumptionReportPdf';
...
const Dashboard = () => {
    ...
    }, [receipts, products, selectedProductNames]);
    ...
    const handleGenerateConsumptionReport = async () => {
        if (!stats || isGeneratingReport) return;

        setIsGeneratingReport(true);
        setReportFeedback('');

        try {
            await generateConsumptionAnalysisPdf({ receipts, products, stats });
            setReportFeedback('Relatório PDF gerado com sucesso.');
        } catch (error) {
            console.error('Erro ao gerar relatório PDF:', error);
            setReportFeedback('Não foi possível gerar o relatório PDF.');
        } finally {
            ...
            <div className="glass-card" style={{ marginBottom: '25px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <button
                        ...
                        style={{
                            background: isGeneratingReport ? 'linear-gradient(135deg, #94A3B8 0%, #CBD5E1 100%)' : 'linear-gradient(135deg, #1F3A5F 0%, #4A90C2 100%)',
                            opacity: isGeneratingReport ? 0.9 : 1
                        }}
                    >
