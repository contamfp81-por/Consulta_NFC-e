...
import { withInsightWhy } from '../utils/insightNarrative';
...
const buildPeriodComparisonSummary = ({ receipts = [], products = [] } = {}) => {
    const totalSpent = receipts.reduce((sum, receipt) => sum + (Number(receipt.totalValue) || 0), 0);
    const transactionsCount = receipts.length;
    const activeDays = new Set(receipts.map((receipt) => getDateKey(receipt.date)).filter(Boolean)).size;
    const averageTicket = transactionsCount > 0 ? totalSpent / transactionsCount : 0;
    const categoryTotals = new Map();
    const granularProducts = products.filter((product) => !isPixSyntheticProduct(product));

    granularProducts.forEach((product) => {
        const category = product.category || 'Outros';
        categoryTotals.set(category, (categoryTotals.get(category) || 0) + (Number(product.totalValue) || 0));
    });
    ...
    return {
        ...
        activeDays,
        averageTicket,
        itemCount: granularProducts.length,
        topCategory
    };
};

const buildComparisonMetric = (label, currentValue, previousValue) => {
    const deltaValue = currentValue - previousValue;
    const deltaRatio = previousValue > 0 ? (deltaValue / previousValue) : 0;
    const tone = deltaValue > 0 ? 'negative' : deltaValue < 0 ? 'positive' : 'neutral';
...
const Dashboard = ({ onOpenForecastPrecision, dateFilter = {}, comparisonFilter = null, comparisonMode = 'none' }) => {
    ...
    const pixCoverageNotes = useMemo(() => ({
        productAnalytics: hasPixSyntheticExpenses
            ? 'Graficos baseados em produtos usam apenas itens comparaveis do cupom. Despesas via QR Pix continuam no total do periodo, mas ficam fora dessas leituras porque nao representam um produto real com historico de preco.'
            : '',
    ...
    const normalizedDateRange = useMemo(() => {
        const { startDate, endDate } = normalizeDateRange(startDateFilter, endDateFilter);
        return {
            start: startDate,
            end: endDate
        };
    ...
    const granularFilteredProducts = useMemo(
        () => filteredProducts.filter((product) => !isPixSyntheticProduct(product)),
        [filteredProducts]
    );
    const comparisonReceipts = useMemo(() => {
        if (!comparisonFilter?.startDate || !comparisonFilter?.endDate) {
            return [];
        }

        return receipts.filter((receipt) => isDateKeyWithinRange(getDateKey(receipt.date), comparisonFilter));
    }, [comparisonFilter, receipts]);
    const comparisonReceiptIds = useMemo(
        () => new Set(comparisonReceipts.map((receipt) => receipt.id)),
        [comparisonReceipts]
    );
    const comparisonProducts = useMemo(
        () => products.filter((product) => comparisonReceiptIds.has(product.receiptId)),
        [comparisonReceiptIds, products]
    );
    const currentPeriodSummary = useMemo(
        () => buildPeriodComparisonSummary({ receipts: filteredReceipts, products: filteredProducts }),
        [filteredProducts, filteredReceipts]
    );
    const comparisonPeriodSummary = useMemo(
        () => (comparisonReceipts.length > 0 ? buildPeriodComparisonSummary({ receipts: comparisonReceipts, products: comparisonProducts }) : null),
        [comparisonProducts, comparisonReceipts]
    );

    const stats = useMemo(() => {
        if (!filteredReceipts.length && !filteredProducts.length) return null;

        const syntheticPixProducts = filteredProducts.filter((product) => isPixSyntheticProduct(product));
        const totalReceiptSum = filteredReceipts.reduce((sum, receipt) => sum + (Number(receipt.totalValue) || 0), 0);
        const totalDetailedProductSum = filteredProducts.reduce((sum, product) => sum + (Number(product.totalValue) || 0), 0);
        const totalProductSum = granularFilteredProducts.reduce((sum, product) => sum + (Number(product.totalValue) || 0), 0);
        const syntheticPixTotal = syntheticPixProducts.reduce((sum, product) => sum + (Number(product.totalValue) || 0), 0);
        const receiptItemGap = totalReceiptSum - totalDetailedProductSum;
        ...
        const prodMap = {};
        const recurrenceMap = {};
        granularFilteredProducts.forEach(p => {
        ...
        granularFilteredProducts.forEach((product) => {
        ...
        return {
            ...
            detailedItemSpent: totalDetailedProductSum,
            productTotalSpent: totalProductSum,
            receiptItemGap,
            detailedItemCount: filteredProducts.length,
            comparableProductCount: granularFilteredProducts.length,
            syntheticPixProductCount: syntheticPixProducts.length,
            syntheticPixProductTotal: syntheticPixTotal,
            storeData,
            dailyEvolutionData,
            stackedData,
            accumulatedData,
    ...
    }, [filteredProducts, filteredReceipts, granularFilteredProducts, productGrouping, selectedProductGroupIds]);
    ...
    const currentMonthInsight = useMemo(() => {
        ...
        let paceDescription = withInsightWhy(
            'O ritmo recente esta alinhado com a media observada desde o inicio do mes',
            'a media da janela mais recente ficou perto do comportamento acumulado do proprio mes'
        );

        if (paceChange > 0.12) {
            paceLabel = 'Acelerando';
            paceColor = '#D84315';
            paceDescription = withInsightWhy(
                `Os ultimos ${recentWindow.length} dias ficaram ${formatSharePercent(paceChange)} acima da janela imediatamente anterior`,
                'o gasto recente acelerou em relacao ao bloco anterior e aumentou a pressao sobre o fechamento do mes'
            );
        } else if (paceChange < -0.12) {
            paceLabel = 'Desacelerando';
            paceColor = '#2E7D32';
            paceDescription = withInsightWhy(
                `Os ultimos ${recentWindow.length} dias recuaram ${formatSharePercent(Math.abs(paceChange))} versus a janela anterior`,
                'a janela mais recente perdeu intensidade frente ao bloco anterior e reduziu a pressao do mes'
            );
        }
        ...
        return {
            ...
            profile,
            outlookSentence: withInsightWhy(
                outlookSentence,
                `a projecao considera o ritmo observado ate agora e ainda estima mais ${formatCurrencyValue(projectedAdditionalSpend)} ate o ultimo dia`
            ),
            concentrationSentence: withInsightWhy(
                concentrationSentence,
                topCategory && topStore
                    ? 'categoria lider e estabelecimento lider absorvem a maior parte do valor e mostram onde o orcamento esta mais exposto'
                    : 'o gasto do mes esta mais espalhado e sem um unico polo dominante de consumo'
            ),
            paymentSentence: withInsightWhy(
                paymentSentence,
                dominantPaymentMethod
                    ? 'essa forma concentrou a maior parte da liquidacao do mes e ajuda a explicar como o gasto esta sendo financiado'
                    : 'os lancamentos estao mais distribuidos entre os meios usados ate agora'
            ),
            actionSentence: withInsightWhy(
                actionSentence,
                paceChange > 0.12
                    ? 'a curva recente acelerou e esses grupos devem puxar a maior parte do valor adicional ate o fim do mes'
                    : essentialShare >= 0.62
                        ? 'a maior parte do gasto esta em categorias essenciais e pequenos ajustes de habito podem preservar margem sem cortar itens-chave'
                        : 'o padrao do mes esta mais distribuido e o controle fino depende dos polos que mais movimentam valor'
            ),
            todaySpent,
            todayPredictedSpend,
    ...
    const averageTicketValue = hasFilteredResults
        ...
        : 0;
    const hasReceiptItemGap = hasFilteredResults && Math.abs(stats.receiptItemGap) > 0.01;
    const dashboardBaseNote = hasFilteredResults
        ? [
            `Itens comparaveis nos graficos de produto: ${formatCurrencyValue(stats.productTotalSpent)} em ${stats.comparableProductCount} registro(s).`,
            stats.syntheticPixProductTotal > 0
                ? `${formatCurrencyValue(stats.syntheticPixProductTotal)} de Pix sintetico permanecem no total do periodo, mas ficam fora dos graficos de produto.`
                : '',
            hasReceiptItemGap
                ? `A soma dos cupons difere da soma dos itens em ${formatCurrencyValue(Math.abs(stats.receiptItemGap))}, normalmente por descontos, arredondamentos ou ajustes do cupom.`
                : ''
        ].filter(Boolean).join(' ')
        : '';
    const comparisonLabel = comparisonFilter?.startDate && comparisonFilter?.endDate
        ? `${formatFilterDateLabel(comparisonFilter.startDate)} até ${formatFilterDateLabel(comparisonFilter.endDate)}`
        : '';
    const comparisonMetrics = comparisonPeriodSummary ? [
        buildComparisonMetric('Gastos', currentPeriodSummary.totalSpent, comparisonPeriodSummary.totalSpent),
        buildComparisonMetric('Lancamentos', currentPeriodSummary.transactionsCount, comparisonPeriodSummary.transactionsCount),
        buildComparisonMetric('Ticket medio', currentPeriodSummary.averageTicket, comparisonPeriodSummary.averageTicket),
        buildComparisonMetric('Dias ativos', currentPeriodSummary.activeDays, comparisonPeriodSummary.activeDays)
    ] : [];
        ...
        <div className="animate-slide-up" style={{ paddingBottom: '20px' }}>
            ...
            <div className="glass-card" style={{ background: 'var(--accent-gradient)', color: 'white', marginBottom: '25px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                    <div>
                        <p style={{ opacity: 0.8, fontSize: '0.9rem' }}>Total de lancamentos no periodo</p>
                ...
                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginTop: '15px', paddingTop: '15px', borderTop: '1px solid rgba(255,255,255,0.2)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Hash size={16} /> {stats.detailedItemCount} itens detalhados</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><ShoppingBag size={16} /> {stats.comparableProductCount} comparaveis</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><TrendingUp size={16} /> R$ {averageTicketValue.toFixed(2)} por cupom</div>
                </div>
                {dashboardBaseNote && (
                    <div style={{ marginTop: '12px', fontSize: '0.8rem', opacity: 0.88, lineHeight: 1.5 }}>
                        {dashboardBaseNote}
                    </div>
                )}
                    ...
                    <div style={{ marginTop: '15px', paddingTop: '15px', borderTop: '1px solid rgba(255,255,255,0.2)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: '12px' }}>
                            <div style={{ fontSize: '0.82rem', opacity: 0.85 }}>
                                Comparando com {COMPARISON_MODE_LABELS[comparisonMode] || 'periodo anterior'}
                            </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px' }}>
                            {comparisonMetrics.map((metric) => (
                                ...
                                >
                                    <div style={{ fontSize: '0.72rem', opacity: 0.8, marginBottom: '6px', textTransform: 'uppercase' }}>{metric.label}</div>
                                    <div style={{ fontSize: '1.05rem', fontWeight: 800, marginBottom: '4px' }}>
                                        {metric.label === 'Gastos' || metric.label === 'Ticket medio'
                                            ? formatCurrencyValue(metric.currentValue)
                                            : metric.currentValue}
                                    </div>
                                    <div
                                        style={{
                                            fontSize: '0.78rem',
                                            color: metric.tone === 'positive' ? '#BBF7D0' : metric.tone === 'negative' ? '#FECACA' : 'rgba(255,255,255,0.8)'
                                        }}
                                    >
                                        {metric.hasBase
                                            ? `${metric.deltaRatio > 0 ? '+' : ''}${(metric.deltaRatio * 100).toFixed(1)}% vs comparativo`
                                            : 'Sem base para comparar'}
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div style={{ marginTop: '12px', fontSize: '0.8rem', opacity: 0.88 }}>
                            Categoria lider no comparativo: {comparisonPeriodSummary.topCategory?.name || 'Sem destaque'}.
                        </div>
                    </div>
                )}
            </div>
            ...
            <>
                    ...
                    <ChartCard title="Distribuição por Loja (%)" icon={PieIcon} contentHeight={MEDIUM_CHART_HEIGHT}>
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                ...
                                <Tooltip formatter={(v) => `${((Number(v) / Math.max(1, stats.totalSpent)) * 100).toFixed(1)}%`} />
                                <Legend
                                    verticalAlign="bottom"
                                    align="center"
                ...
                <ChartCard title="Gráfico de Pareto (Top 10 Produtos)" icon={Target} fullWidth contentHeight="auto">
                    ...
                    {pixCoverageNotes.productAnalytics && (
                        <div style={{ marginTop: '12px', fontSize: '0.78rem', color: 'var(--text-light)', lineHeight: 1.5 }}>
                            {pixCoverageNotes.productAnalytics}
                        </div>
                    )}
