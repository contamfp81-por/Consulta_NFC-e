...
const HistoryView = ({ dateFilter = {} }) => {
    ...
    const normalizedDateRange = useMemo(
        () => normalizeDateRange(dateFilter.startDate, dateFilter.endDate),
    ...
    const filteredReceipts = receipts.filter((receipt) => (
        isDateKeyWithinRange(toLocalDateKey(receipt.date), normalizedDateRange)
        && matchesSearch([
    ...
    const filteredPixExpenses = pixExpenses.filter((pixExpense) => (
        isDateKeyWithinRange(toLocalDateKey(pixExpense.date), normalizedDateRange)
        && matchesSearch([
            pixExpense.receiver,
            pixExpense.category,
    ...
    const hasVisibleEntries = filteredReceipts.length > 0 || filteredPixExpenses.length > 0;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginTop: '5px', fontSize: '0.75rem', color: 'var(--text-light)' }}>
                                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                        <Calendar size={12} /> {new Date(receipt.date).toLocaleDateString('pt-BR')}
                                    </span>
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginTop: '5px', fontSize: '0.75rem', color: 'var(--text-light)' }}>
                                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                        <Calendar size={12} /> {new Date(pixExpense.date).toLocaleDateString('pt-BR')}
                                    </span>
                                    <span className="badge badge-success">
                                ...
                                >
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
                                        {[
                                            ...
                                            <div key={item.label} style={{ padding: '12px', borderRadius: '12px', background: 'rgba(8, 19, 31, 0.66)', border: '1px solid rgba(148, 163, 184, 0.12)' }}>
                                    ...
                                    <div style={{ marginTop: '14px', display: 'grid', gap: '12px' }}>
                                        ...
                                        {(pixExpense.description || pixExpense.observation) && (
                                            <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(8, 19, 31, 0.66)', border: '1px solid rgba(148, 163, 184, 0.12)' }}>
                ...
                {!hasVisibleEntries && (
                    <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-light)' }}>
                        ...
                        <p>{historyEntries.length === 0 ? 'Nenhum cupom ou despesa ainda.' : 'Nenhum lancamento encontrado no intervalo atual.'}</p>
                        <p style={{ fontSize: '0.8rem' }}>
                            {historyEntries.length === 0
                                ? 'Escaneie um cupom, use a aba Manual ou restaure uma copia de seguranca.'
                                : 'Ajuste a busca ou o periodo de analise para localizar os lancamentos desejados.'}
                        </p>
                    </div>
                )}
            </div>
