...
const SpendingTree = ({ onBack }) => {
    ...
    // Sync data to ref for the ticker to access without stale closures
    useEffect(() => {
        dataRef.current = { monthData, budget, usagePercent };
    }, [monthData, budget, usagePercent]);
    ...
    useEffect(() => {
        ...
        const initPixi = async () => {
            ...
            app.ticker.add((ticker) => {
                time += 0.02;
                const { monthData: currentData, budget: currentBudget, usagePercent: currentUsage } = dataRef.current;
                renderTree(container, currentData, currentBudget, currentUsage, time);
            });
    ...
    const renderTree = (container, data, budget, usage, time) => {
        ...
        // Background Nature (Simplified for performance)
        const bg = new Graphics();
        bg.beginFill(0xf0f9ff); // Light sky blue
