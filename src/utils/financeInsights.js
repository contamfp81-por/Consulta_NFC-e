...
export const buildFinanceOverview = ({
    ...
    if (variation.direction === 'up' && variation.value > 0.05) {
        financeAlerts.push(`Seus gastos subiram ${(variation.value * 100).toFixed(1)}% em relacao ao mes anterior.`);
    }
    if (healthScore.score < 60 && currentMonthTotal > 0) {
        financeAlerts.push('Seu indice de saude financeira esta alertando perigo. Reveja seu ritmo de saidas.');
    } else if (healthScore.score >= 85 && currentMonthTotal > 0) {
        financeAlerts.push('Otimo comportamento no mes! Sua saude financeira esta excelente.');
    }
    if (currentCreditShare > 0.5) {
        financeAlerts.push('Cuidado: Mais da metade das despesas deste mes baseiam-se em cartao de credito ou prazo longo.');
    }
    if (topCategoryShare > 0.4 && topCategory) {
        financeAlerts.push(`Alerta de concentracao: A categoria '${topCategory.name}' representa ${(topCategoryShare * 100).toFixed(1)}% do total mensal.`);
    }
    if (pendingPixCount > 0) {
        financeAlerts.push(`Nao esqueca de classificar as ${pendingPixCount} transacao(oes) Pix pendente(s).`);
    }
    if (receiptOriginSummary.manualReceiptsCount > receiptOriginSummary.importedReceiptsCount && receiptOriginSummary.importedReceiptsCount > 5) {
        financeAlerts.push('Dica: Use mais a camera! Escanear o QR Code extrai dados mais ricos (nomes e categorias reais) do que apenas lancar despesas manuais.');
    }
    if (activeDays >= 3 && maxDailySpend > averageActiveDaySpend * 2.5) {
        financeAlerts.push(`Atencao para o salto: Houve um dia atipico com desembolso isolado muito elevado (Pico do mes: ${formatCurrency(maxDailySpend)}).`);
    }
