...
const SUGARY_KEYWORDS = SUGAR_GROUPS.flatMap((group) => group.keywords);
...
const detectSugarGroup = (normalizedText) => {
    const match = SUGAR_GROUPS.find((group) => hasKeyword(normalizedText, group.keywords));
    return match?.group || '';
};

export const identifyFoodClassification = (productName = '', categoryOriginal = '') => {
    ...
    const matchedNatural = NATURAL_KEYWORDS.filter((keyword) => searchableText.includes(keyword));
    const matchedModerate = MODERATE_KEYWORDS.filter((keyword) => searchableText.includes(keyword));
    const matchedUltra = ULTRAPROCESSED_KEYWORDS.filter((keyword) => searchableText.includes(keyword));
...
export const buildFoodPurchaseItems = ({
    ...
    return sourceItems
        .map((item) => {
            const classification = identifyFoodClassification(item.nome, item.categoriaOriginal);
            if (!classification.isFoodCandidate) {
                return null;
            }
...
const buildTrendComparison = (currentSummary, previousSummary, label) => {
    ...
    let message = 'Sua alimentacao manteve um padrao semelhante ao periodo anterior';
...
const buildAutomaticAlerts = ({ monthTrend, weekTrend, sugarAnalysis, impulsivity, behaviorIndicators }) => {
    const alerts = [];

    if (monthTrend.direction === 'worse' || monthTrend.direction === 'better') alerts.push(monthTrend.message);
    if (weekTrend.direction === 'worse' && monthTrend.direction !== 'worse') alerts.push(weekTrend.message);
    if (sugarAnalysis.classification === 'alto') alerts.push('Seu consumo de açucar esta alto.');
...
const buildExecutiveSummary = ({ monthTrend, sugarAnalysis, impulsivity, behaviorIndicators, alerts = [] }) => {
    const lines = [];

    lines.push(monthTrend.message);

    if (sugarAnalysis.classification === 'alto') {
        lines.push('O score de açucar ficou acima da faixa segura e pede revisao das compras doces.');
    } else if (sugarAnalysis.classification === 'moderado') {
        lines.push('O consumo de açucar exige atencao moderada, principalmente em bebidas e confeitaria.');
    } else {
        lines.push('O score de açucar esta sob controle no periodo analisado.');
    }

    if (behaviorIndicators.detectedPatterns.length > 0) lines.push(behaviorIndicators.detectedPatterns[0]);
    if (impulsivity.classification === 'alto') lines.push('O comportamento recente mostra alta probabilidade de compras por impulso.');
    else if (impulsivity.classification === 'moderado') lines.push('Ha sinais moderados de impulsividade que merecem acompanhamento.');
    else lines.push('O padrao de compra esta relativamente estavel contra impulsividade.');
...
export const analyzeFoodPurchases = ({
    ...
    const safeReferenceDate = toDate(referenceDate) || new Date();
    const normalizedItems = buildFoodPurchaseItems({ items, receipts, products, productAliases });
    ...
    return {
        ...
        totals: {
            ...
            totalSpendByLevel: [
                ...
                { name: FOOD_LEVEL_LABELS.unknown, key: 'unknown', value: Math.max(0, overallSummary.totalSpend - overallSummary.naturalSpend - overallSummary.moderateSpend - overallSummary.ultraprocessedSpend) }
            ]
        },
        percentages: {
