...
export const processNFCeURL = async (url) => {
    ...
    try {
        ...
        // 1. CHAVE DE ACESSO (O DNA da nota - 44 dígitos)
        // Tentamos pegar do URL ou do HTML
        let accessKey = "";
        const keyMatch = url.match(/[0-9]{44}/);
        if (keyMatch) {
            accessKey = keyMatch[0];
        } else {
            accessKey = doc.querySelector('.chave, #lblChaveAcesso, .txtChave')?.textContent?.replace(/\s/g, '') || "";
        }

        // 2. NÚMERO DA NOTA
        const nReceipt = doc.querySelector('#lblNF, .txtNF, .nfe-info')?.textContent?.match(/\d+/)?.[0] || "S/N";
        ...
        // 4. DATA E HORA DA COMPRA (Fundamental para o fluxo de gastos)
        let processedDate = new Date(); // Fallback date
        const dateRaw = doc.body.textContent.match(/(\d{2}\/\d{2}\/\d{4})/);
        const timeRaw = doc.body.textContent.match(/(\d{2}:\d{2}:\d{2})/);

        if (dateRaw) {
            const [day, month, year] = dateRaw[1].split('/');
            const timeStr = timeRaw ? timeRaw[1] : '12:00:00';
        ...
        const products = [];
        // Seletores robustos para PR e outros estados
        const rows = doc.querySelectorAll('table#tabResult tr, .container-itens, .NFCCalculadora tr, #infoConsulta tr, .table-itens tr');

        rows.forEach(row => {
            const name = row.querySelector('.txtTit, .description, .item-name, .txtNome, .NM-PRODUTO')?.textContent?.trim();
            const price = row.querySelector('.valor, .unit-price, .txtValUnit, .VL-UNITARIO')?.textContent?.trim()?.replace(',', '.');
            const qty = (row.querySelector('.Rqtd, .quantity, .txtQtde, .QT-ITEM')?.textContent || '1')
                .replace(/[^\d.,]/g, '')
                .replace(',', '.')
                .trim();
