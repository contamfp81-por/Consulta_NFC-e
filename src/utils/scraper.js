...
// Função auxiliar para limpar e converter números do padrão BR (1.234,56) para JS (1234.56)
const parseBrazilianNumber = (text) => {
    ...
    // Se houver só vírgula, é decimal. Ex: 5,50
...
export const processNFCeURL = async (url) => {
    ...
    try {
        ...
        if (!html) throw new Error("Conteúdo não encontrado.");
        ...
        if (keyMatch) {
            ...
            accessKey = doc.querySelector('.chave, #lblChaveAcesso, .txtChave, .nfe-chave, .access-key')?.textContent?.replace(/\s/g, '') || "";
        }

        // 2. NÚMERO DA NOTA
        const nReceipt = doc.querySelector('#lblNF, .txtNF, .nfe-info, .nfe-numero, #lblNumero')?.textContent?.match(/\d+/)?.[0] || "S/N";

        // 3. ESTABELECIMENTO (SEFAZ PR usa muito #lblNomeFantasia ou .txtTopo)
        const establishment =
            ...
            doc.querySelector('.nf-estabelecimento') ||
            doc.querySelector('#lblRazaoSocial') ||
            { textContent: "Estabelecimento via NFC-e" };

        // 4. DATA E HORA DA COMPRA
        let processedDate = new Date();
        // Busca padrão de data no corpo: 01/01/2024
        ...
        // 5. VALOR TOTAL DA NOTA (SEFAZ PR usa muito #lblVlrTotal)
        const totalValueStr =
            doc.querySelector('#lblVlrTotal') ||
            doc.querySelector('.txtMax') ||
            doc.querySelector('.totalNFe') ||
            doc.querySelector('.vlrTotal') ||
            doc.querySelector('.nf-total') ||
            doc.querySelector('#ValorTotal') ||
            { textContent: "0,00" };

        const products = [];
        // Seletores de linhas para o portal PR e outros
        const rows = doc.querySelectorAll('table#tabResult tr, .container-itens, .NFCCalculadora tr, #infoConsulta tr, .table-itens tr, .lineItem, .nf-item, #itens tr, #tabelaItens tr');
        ...
        rows.forEach(row => {
            ...
            if (nameEl && (priceEl || qtyEl)) {
                ...
                const rawQty = qtyEl ? qtyEl.textContent : '1';
                const unit = unitEl ? unitEl.textContent.trim().toUpperCase() : 'UN';

                const pUnit = parseBrazilianNumber(rawPrice);
                const pQty = parseBrazilianNumber(rawQty);
        ...
        // Refinamento específico para tabelas do PR (caso o acima falhe)
        if (products.length === 0) {
            doc.querySelectorAll('table tr').forEach(tr => {
                const text = tr.innerText || tr.textContent;
                // Procura por linhas que pareçam ter um preço (R$ ou número com vírgula no fim)
                if (text.includes(',') && (text.includes('UN') || text.includes('KG') || text.includes('LT'))) {
                    const tds = tr.querySelectorAll('td');
                    if (tds.length >= 3) {
                        const name = tds[0].textContent.trim();
                        // Tenta extrair quantidade da 2ª ou 3ª coluna
                        const qty = tds[1].textContent.trim();
                        const val = tds[tds.length - 1].textContent.trim();

                        const pQty = parseBrazilianNumber(qty);
                        const pVal = parseBrazilianNumber(val);

                        if (name.length > 2 && pVal > 0) {
                            products.push({
                                name: name,
                                brand: "Marca",
                                quantity: pQty || 1,
                                unit: "UN",
                                unitPrice: pVal / (pQty || 1),
                                totalValue: pVal,
                                category: autoCategorize(name)
                            });
                        }
        ...
        return {
            ...
            totalValue: parseBrazilianNumber(totalValueStr.textContent),
            accessKey: accessKey,
