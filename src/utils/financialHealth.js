    ...
    if (!current || !previous) {
        return {
            ...
            message: withInsightWhy(
                'Continue registrando meses completos para destravar uma projecao mais precisa',
                'sem dois ciclos fechados o modelo ainda nao consegue comparar tendencia recente com consistencia historica'
            )
        };
    }
    ...
    return {
        ...
        message: delta < -2
            ? withInsightWhy(
                ...
                `o ultimo resultado ficou ${Math.abs(delta)} ponto(s) abaixo do ciclo anterior e essa trajetoria projeta nova perda`
            )
            : delta > 2
                ? withInsightWhy(
                    `Mantendo o ritmo atual, seu indice pode avancar para ${projectedNextScore} no proximo mes`,
                    `o ultimo resultado ganhou ${Math.abs(delta)} ponto(s) frente ao ciclo anterior e reforca uma tendencia positiva`
                )
                : withInsightWhy(
                    'O comportamento recente indica estabilidade para o proximo ciclo',
                    'a diferenca entre os ultimos meses foi pequena e nao aponta mudanca brusca na estrutura do ISF'
                )
    };
};
...
const buildPrimaryDriverText = (component) => {
    if (!component) {
        return withInsightWhy(
            'Sem dados suficientes para identificar o principal vetor do indice',
            'a base atual ainda nao reuniu elementos suficientes para separar o bloco que mais pesa no resultado'
        );
    }
    ...
    if (component.key === 'alimentacao') {
        if (component.ultraPenalty > 0) {
            return withInsightWhy(
                'O principal freio do ISF veio da alimentacao',
                `ultraprocessados responderam por ${component.ultraprocessedShare.toFixed(1)}% do gasto alimentar e derrubaram a nota desse bloco`
            );
        }

        if (component.spendPenalty > 0) {
            return withInsightWhy(
                'A alimentacao pressionou o indice por volume',
                `o gasto ficou ${Math.round(component.spendIncreaseRatio * 100)}% acima da media recente e elevou a penalidade do ciclo`
            );
        }
    }
    ...
    if (component.key === 'essenciais' && component.penalty > 0) {
        return withInsightWhy(
            'Os essenciais reduziram a previsibilidade financeira',
            `agua e energia subiram ${Math.round(component.increaseRatio * 100)}% sobre a media e comprimiram a margem do mes`
        );
    }

    if (component.key === 'discricionario' && component.overspendRatio > 0) {
        return withInsightWhy(
            'As despesas discricionarias comprimiram a margem do mes',
            `esse bloco ultrapassou o orcamento inferido em ${Math.round(component.overspendRatio * 100)}%`
        );
    }

    return withInsightWhy(
        `${component.label} sustentou bem o indice e ajudou a amortecer riscos do ciclo`,
        `esse bloco terminou o periodo com score ${component.score}/100 e ajudou a compensar as demais pressoes`
    );
};
...
export const gerarInsights = ({
    ...
    if (currentMonth?.trendDelta > 0) {
        insights.push(withInsightWhy(
            `Seu ISF melhorou ${Math.abs(currentMonth.trendDelta)} ponto(s) versus o periodo anterior`,
            'o conjunto entre alimentacao, essenciais e discricionario fechou este ciclo com menos pressao que o anterior'
        ));
    } else if (currentMonth?.trendDelta < 0) {
        insights.push(withInsightWhy(
            `Seu ISF caiu ${Math.abs(currentMonth.trendDelta)} ponto(s) versus o periodo anterior`,
            'o ciclo atual perdeu equilibrio em relacao ao anterior e isso reduziu a pontuacao consolidada'
        ));
    }
    ...
    if (currentMonth?.essenciais?.penalty > 0) {
        insights.push(withInsightWhy(
            `Energia e agua somadas ficaram em ${Math.round(currentMonth.essenciais.increaseRatio * 100)}% acima da media usada como referencia`,
            'o bloco de essenciais subiu acima do historico recente e perdeu previsibilidade'
        ));
    } else if (currentMonth?.discricionario?.score >= 100) {
        insights.push(withInsightWhy(
            'Voce manteve controle nas despesas variaveis e protegeu o indice no bloco discricionario',
            'os gastos opcionais ficaram dentro do orcamento inferido para o ciclo'
        ));
    }
...
export const construirIndiceSaudeFinanceira = ({
    ...
    const currentMonthReceipts = receipts.filter((receipt) => getMonthKey(receipt.date) === currentMonthKey);
    const currentMonthReceiptIds = new Set(currentMonthReceipts.map((receipt) => receipt.id));
    const currentMonthProducts = products.filter((product) => currentMonthReceiptIds.has(product.receiptId));
    const currentCycleFoodAnalysis = analyzeFoodPurchases({
    ...
    const insights = gerarInsights({
        ...
        projection,
        foodAnalysis: currentCycleFoodAnalysis
    });
    ...
    if (!currentMonth?.hasData) {
        return {
            ...
            insights: [withInsightWhy(
                'Quando os primeiros gastos entrarem, o card vai explicar automaticamente o que puxa seu indice para cima ou para baixo',
                'o ISF depende de movimentacao real para comparar alimentacao, essenciais e despesas variaveis'
            )],
            ...
            projection: {
                ...
                delta: 0,
                message: withInsightWhy(
                    'Sem historico suficiente para projetar o proximo mes',
                    'a previsao so fica confiavel quando ha meses fechados suficientes para comparar tendencia e consistencia'
                )
            },
    ...
    return {
        ...
        transparencyNote: currentCycleFoodAnalysis?.meta?.pendingReviewCount > 0
            ? `Ha ${currentCycleFoodAnalysis.meta.pendingReviewCount} item(ns) alimentares aguardando classificacao manual, o que pode refinar o ISF.`
