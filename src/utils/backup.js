import { db } from '../db';
import { buildPixExpenseIdentityKey } from './qrCode';
...
const normalizePixExpenses = (rows = []) => {
    ...
    const normalizedRows = rows
        ...
        .reduce((accumulator, row) => {
            const expenseKey = buildPixExpenseIdentityKey(row);
    ...
    return {
        rows: normalizedRows,
        duplicatesRemoved: Math.max(0, rows.length - normalizedRows.length)
    };
};
...
const normalizeBackupPayload = (payload) => {
    ...
    const normalizedPixExpenses = normalizePixExpenses(rawTables.pixExpenses || []);
    const tables = {
        ...rawTables,
        pixExpenses: normalizedPixExpenses.rows
    };
    ...
    return {
        ...
        tableNames,
        tables,
        pixExpensesDuplicatesRemoved: normalizedPixExpenses.duplicatesRemoved
    };
};
...
export const exportDatabaseBackup = async () => {
    ...
    const rawTables = Object.fromEntries(tableEntries);
    const normalizedPixExpenses = normalizePixExpenses(rawTables.pixExpenses || []);
    const tables = {
        ...rawTables,
        pixExpenses: normalizedPixExpenses.rows
    ...
    return {
        ...
        pixExpenseCount: counts.pixExpenses || 0,
        pixExpenseDuplicatesRemoved: normalizedPixExpenses.duplicatesRemoved
    };
};
...
export const importDatabaseBackup = async (source) => {
    ...
    return {
        ...
        totalRecords: sumCounts(counts),
        pixExpenseCount: counts.pixExpenses || 0,
        pixExpenseDuplicatesRemoved: normalizedPayload.pixExpensesDuplicatesRemoved || 0
    };
};
