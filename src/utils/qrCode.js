...
export const buildPixExpenseIdentityKey = ({
    ...
    const dateKey = normalizedDate || 'SEM_DATA';
    const shouldScopeByDate = !normalizedTxid && normalizedPayloadType !== 'DINAMICO';

    if (normalizedPayload) {
        return shouldScopeByDate
            ? ['payload', normalizedPayload, dateKey].join('|')
            : ['payload', normalizedPayload].join('|');
    }
