...
const withInsightSectionWhy = (section = {}, reasons = {}) => ({
    ...section,
    comportamento: section?.comportamento
        ? withInsightWhy(section.comportamento, reasons.comportamento || '')
        : section?.comportamento,
    tendencia: section?.tendencia
        ? withInsightWhy(section.tendencia, reasons.tendencia || '')
        : section?.tendencia
});
...
const buildInsights = (reportData) => {
    ...
    const sectionWhyByKey = {
        projection: {
            comportamento: reportData.monthInsight?.hasData
                ? 'o calculo usa o realizado ate agora, o ritmo diario e o historico do proprio mes para estimar o fechamento'
                : 'sem lancamentos do mes vigente nao ha base para projetar o fechamento com confianca',
            tendencia: reportData.monthInsight?.hasData
                ? 'o ritmo diario recente define se o fechamento tende a acelerar ou permanecer dentro das margens historicas'
                : 'a tendencia so aparece quando o mes corrente acumula dados suficientes'
        },
        productEvolution: {
            comportamento: reportData.productEvolutionData.length > 1
                ? 'o relatorio comparou a cesta monitorada entre datas diferentes e mediu a variacao real do preco medio'
                : 'sem repeticao dos mesmos itens em datas diferentes nao ha serie para comparar',
            tendencia: reportData.productEvolutionData.length > 1
                ? 'a direcao da curva mostra se o custo unitario medio esta ganhando ou perdendo pressao ao longo do tempo'
                : 'a tendencia depende de pelo menos dois registros comparaveis do mesmo grupo'
        },
        temporal: {
            comportamento: peakTemporal
                ? 'esse pico ficou acima da media diaria e revela um ponto de maior sensibilidade no fluxo'
                : 'a base temporal ainda nao tem concentracao suficiente para uma leitura forte',
            tendencia: 'a projecao dos proximos 30 dias usa o ritmo agregado para indicar se o gasto tende a subir ou aliviar'
        },
        storeDistribution: {
            comportamento: topStore
                ? 'essa loja absorveu a maior parcela do valor registrado e ganhou dominancia na sua carteira de compra'
                : 'a base ainda nao mostra uma loja dominante',
            tendencia: topStoreShare > 50
                ? 'uma loja concentrando mais da metade do valor aumenta a exposicao a variacao de preco e conveniencia'
                : 'a distribuicao entre lojas ainda preserva margem para comparar preco e oportunidade'
        },
        storeComparison: {
            comportamento: topStore && secondStore
                ? 'a diferenca entre a primeira e a segunda loja mostra onde existe maior concentracao de gasto'
                : 'sem pelo menos duas lojas relevantes nao ha base para comparacao direta',
            tendencia: 'a loja lider tende a ser sua referencia principal e por isso concentra o maior potencial de economia'
        },
        storeTimeline: {
            comportamento: peakMonth
                ? 'esse foi o mes em que as lojas receberam o maior volume e por isso define o pico da serie'
                : 'a serie mensal ainda esta curta para um pico confiavel',
            tendencia: lastMonth
                ? 'comparar o ultimo mes com a media mostra se a rede de lojas esta entrando em fase de alta ou de alivio'
                : 'a tendencia depende de mais meses fechados'
        },
        accumulated: {
            comportamento: totalAccumulated > 0
                ? 'o acumulado mostra em que velocidade o valor total foi sendo construido ao longo do periodo'
                : 'sem acumulado material ainda nao ha leitura do ritmo',
            tendencia: 'a inclinacao da curva resume se o gasto esta acelerando ou desacelerando no fechamento do periodo'
        },
        pareto: {
            comportamento: topParetoItem
                ? 'os primeiros itens concentram a maior parcela do Top 10 e definem onde o corte gera mais impacto'
                : 'o ranking ainda nao encontrou um item dominante',
            tendencia: 'o topo do grafico revela onde pequenas reducoes percentuais entregam a maior economia absoluta'
        },
        treemap: {
            comportamento: topCategory
                ? 'o tamanho visual da categoria lider espelha a importancia financeira dela no orcamento'
                : 'a hierarquia visual depende de uma categoria com peso suficiente',
            tendencia: 'categorias maiores no mapa merecem monitoramento porque concentram a maior parte do valor'
        },
        categories: {
            comportamento: topCategory
                ? 'a categoria ancora puxou a maior participacao percentual e definiu o centro de custo dominante'
                : 'sem categoria lider a distribuicao ainda esta dispersa',
            tendencia: 'a concentracao nas 3 principais categorias mostra se o perfil esta mais focado ou mais espalhado'
        },
        recurrentProducts: {
            comportamento: reportData.topRecurringProducts.length
                ? 'a repeticao desse item ao longo do periodo mostra um habito consolidado de consumo'
                : 'a base ainda nao encontrou itens com recorrencia forte',
            tendencia: 'itens muito recorrentes costumam concentrar oportunidade de economia em promocao, volume ou substituicao'
        },
        priceHistogram: {
            comportamento: dominantPriceBucket
                ? 'essa faixa reune a maior quantidade de itens e define a densidade da sua cesta'
                : 'o histograma precisa de mais itens para destacar uma faixa dominante',
            tendencia: 'a parcela de itens baratos ou caros ajuda a entender se o consumo esta pulverizado ou concentrado'
        },
        paymentMethods: {
            comportamento: topPaymentMethod
                ? 'esse meio concentrou mais valor e apareceu em mais categorias do que os demais'
                : 'a base ainda nao tem meios suficientes para leitura de concentracao',
            tendencia: topPaymentMethod
                ? 'quando um unico meio concentra grande parte do valor, ele passa a influenciar limite, custo financeiro e comportamento'
                : 'a tendencia depende de novos cupons com meio de pagamento identificado'
        },
        composition: {
            comportamento: topCategory
                ? 'a maior fatia do circulo aponta para a categoria que mais desequilibra ou ancora o orcamento'
                : 'o grafico depende de uma categoria dominante para leitura forte',
            tendencia: 'o equilibrio entre fatias mostra se o orcamento esta mais voltado a necessidade ou a desejo'
        },
        flow: {
            comportamento: topLink
                ? 'esse fluxo concentrou o maior valor entre origem e destino e revela o principal canal do seu gasto'
                : 'o diagrama ainda nao encontrou um fluxo dominante',
            tendencia: 'fluxos repetidos mostram fidelidade de compra e criam habitos que podem esconder alternativas melhores'
        },
        heatmap: {
            comportamento: dominantDay
                ? 'esse dia acumulou mais registros do que os outros e indica seu padrao principal de compra'
                : 'a distribuicao semanal ainda nao consolidou um dia dominante',
            tendencia: 'a participacao do fim de semana mostra o peso de conveniencia e de reposicao fora do meio da semana'
        },
        ticketHistogram: {
            comportamento: dominantTicketBucket
                ? 'essa faixa concentrou o maior numero de cupons e define o tamanho mais comum das suas idas ao mercado'
                : 'o histograma ainda nao encontrou um tamanho de ticket dominante',
            tendencia: 'muitos cupons baixos costumam indicar reposicao frequente e mais custo indireto de deslocamento'
        },
        inflation: {
            comportamento: reportData.inflation.comparableProductsCount > 0
                ? 'os produtos comparaveis mostraram como a variacao de preco afetou a sua cesta real, e nao uma media generica do mercado'
                : 'sem repeticao dos mesmos produtos nao ha base para medir inflacao pessoal',
            tendencia: 'a pressao nos itens lideres indica onde a troca de marca, loja ou momento de compra mais protege poder de compra'
        }
    };
    ...
    return {
        ...
        summary: {
            ...
            highlights: [
                withInsightWhy(
                    rawInsights.summary.highlights[0],
                    topCategory
                        ? 'essa categoria concentrou a maior fatia dos itens analisados e puxou o valor total do periodo'
                        : 'a base ainda nao tem volume suficiente para indicar uma categoria lider com confianca'
                ),
                withInsightWhy(
                    rawInsights.summary.highlights[1],
                    topStore
                        ? 'esse estabelecimento concentrou a maior parte do valor dos cupons e ganhou peso acima dos demais'
                        : 'os registros atuais nao mostram dominancia clara entre lojas'
                ),
                withInsightWhy(
                    rawInsights.summary.highlights[2],
                    reportData.inflation.comparableProductsCount > 0
                        ? 'os mesmos produtos se repetiram em datas diferentes e permitiram medir o impacto real do preco no seu bolso'
                        : 'a comparacao de preco ainda precisa de repeticao dos mesmos itens em momentos diferentes'
                )
            ],
            dataIntegrityNote: withInsightWhy(
                rawInsights.summary.dataIntegrityNote,
                hasReceiptItemGap
                    ? 'cupons e itens nao fecharam exatamente o mesmo total e a diferenca normalmente vem de desconto, arredondamento ou ajuste fiscal'
                    : 'o total dos cupons e o total dos itens ficaram coerentes na base usada pelo relatorio'
            ),
            concentrationNote: withInsightWhy(
                rawInsights.summary.concentrationNote,
                topThreeStoreShare > 0
                    ? `as 3 maiores lojas concentram ${formatPercent(topThreeStoreShare)} do orcamento e determinam grande parte da sua exposicao a preco`
                    : 'a distribuicao entre lojas ainda esta em formacao'
            ),
            recurrenceNote: withInsightWhy(
                rawInsights.summary.recurrenceNote,
                peakTemporal
                    ? 'esse dia ficou muito acima da media diaria do periodo e merece leitura como estoque, sazonalidade ou impulso'
                    : 'a serie temporal ainda nao tem repeticao suficiente para destacar um pico confiavel'
            ),
            inflationNote: withInsightWhy(
                rawInsights.summary.inflationNote,
                reportData.inflation.comparableProductsCount > 0
                    ? 'a categoria lider da pressao de preco acumulou o maior impacto financeiro entre os itens comparaveis'
                    : 'a inflacao pessoal depende da repeticao dos mesmos produtos ao longo do tempo'
            )
        },
        ...Object.fromEntries(
            Object.entries(sectionWhyByKey).map(([key, reasons]) => [
                key,
                withInsightSectionWhy(rawInsights[key], reasons)
            ])
        )
    };
};
