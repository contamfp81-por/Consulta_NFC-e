...
const buildReportData = ({ receipts, products, stats, selectedProductGroups }) => {
    ...
    const productEvolutionData = Array.isArray(stats?.productEvolutionData)
        ? stats.productEvolutionData
            .map((item) => ({
                date: item.date,
...
const buildInsights = (reportData) => {
    ...
    const peakMonthTopStore = peakMonth
        ? reportData.topStores
            .map((store) => ({
                label: store.label,
                value: Number(reportData.stackedData.find((item) => item.month === peakMonth.month)?.[store.key]) || 0
            }))
    ...
    const paretoTopFiveShare = reportData.paretoData[4]?.percentage || reportData.paretoData.at(-1)?.percentage || 0;
    ...
    const highTicketShare = safeRatio(
        sum(reportData.receiptHistogramData.slice(-2).map((item) => item.count)),
        totalReceiptsCount
    ) * 100;
    ...
    return {
        summary: {
            ...
            highlights: [
                topCategory
                    ? `A categoria ${topCategory.name} lidera com ${formatPercent(topCategory.percentage)} do gasto analisado.`
                    : 'N\u00e3o foi identificada uma categoria dominante no per\u00edodo.',
                topStore
                    ? `A loja ${topStore.name} responde por ${formatPercent(topStoreShare)} do total monitorado.`
                    : 'N\u00e3o h\u00e1 concentra\u00e7\u00e3o por loja suficiente para destaque.',
                reportData.inflation.comparableProductsCount > 0
                    ? `A infla\u00e7\u00e3o pessoal estimada foi de ${formatSignedRatioPercent(reportData.inflation.personalInflationRate)}, com impacto de ${formatCurrency(reportData.inflation.totalInflationImpact)}.`
                    : 'Ainda n\u00e3o h\u00e1 base compar\u00e1vel para estimar a infla\u00e7\u00e3o pessoal.'
            ],
            concentrationNote: topThreeStoreShare > 0
                ? `As tr\u00eas lojas mais relevantes concentram ${formatPercent(topThreeStoreShare)} do gasto total, o que sinaliza ${topThreeStoreShare >= 60 ? 'forte depend\u00eancia de poucos estabelecimentos.' : 'uma distribui\u00e7\u00e3o mais pulverizada entre estabelecimentos.'}`
                : 'N\u00e3o h\u00e1 massa cr\u00edtica suficiente para medir concentra\u00e7\u00e3o por loja.',
            recurrenceNote: peakTemporal
                ? `O maior pico di\u00e1rio ocorreu em ${formatDate(peakTemporal.date)}, quando o desembolso alcan\u00e7ou ${formatCurrency(peakTemporal.value)}.`
                : 'Ainda n\u00e3o h\u00e1 recorr\u00eancia suficiente para destacar um pico di\u00e1rio.',
            inflationNote: reportData.inflation.comparableProductsCount > 0
                ? `${reportData.inflation.comparableProductsCount} produtos compar\u00e1veis sustentam a leitura inflacion\u00e1ria do per\u00edodo, com press\u00e3o principal em ${inflationTopCategory?.name || 'categorias ainda dispersas'}.`
                : 'A leitura de infla\u00e7\u00e3o pessoal ainda depende de mais registros de pre\u00e7o para os mesmos produtos.'
        },
        productEvolution: {
            finalidade: 'Acompanha o pre\u00e7o m\u00e9dio dos produtos selecionados no dashboard e evidencia varia\u00e7\u00f5es recentes de valor.',
            comportamento: reportData.selectedProductLabels.length && reportData.productEvolutionData.length
                ? `A sele\u00e7\u00e3o atual re\u00fane ${joinLabels(reportData.selectedProductLabels)}. O pre\u00e7o m\u00e9dio saiu de ${formatCurrency(firstProductPoint.price)} em ${formatDate(firstProductPoint.date)} para ${formatCurrency(lastProductPoint.price)} em ${formatDate(lastProductPoint.date)}.`
                : reportData.selectedProductLabels.length
                    ? 'H\u00e1 produtos selecionados no dashboard, mas ainda n\u00e3o existem pontos suficientes para tra\u00e7ar uma s\u00e9rie hist\u00f3rica confi\u00e1vel.'
                    : 'Nenhum produto foi selecionado no dashboard para compor este gr\u00e1fico.',
            tendencia: reportData.selectedProductLabels.length && reportData.productEvolutionData.length > 1
                ? `A trajet\u00f3ria indica ${describeDirection(productPriceVariation)} de ${formatSignedRatioPercent(productPriceVariation)} no pre\u00e7o m\u00e9dio da sele\u00e7\u00e3o.`
                : 'Sem sele\u00e7\u00e3o compar\u00e1vel, o relat\u00f3rio preserva o card, mas sem base suficiente para projetar tend\u00eancia.'
        },
        temporal: {
            finalidade: 'Mostra a evolu\u00e7\u00e3o do gasto total por dia no per\u00edodo filtrado.',
            comportamento: peakTemporal
                ? `O maior pico ocorreu em ${formatDate(peakTemporal.date)}, com ${formatCurrency(peakTemporal.value)}. Isso marca o dia de maior press\u00e3o or\u00e7ament\u00e1ria do intervalo.`
                : 'Ainda n\u00e3o h\u00e1 volume suficiente para caracterizar o comportamento temporal.',
            tendencia: `A proje\u00e7\u00e3o atual indica ${reportData.temporalProjection.direction}, com gasto m\u00e9dio di\u00e1rio estimado em ${formatCurrency(reportData.temporalProjection.projectedDailyAverage)} e ${formatCurrency(reportData.temporalProjection.projectedNext30)} para os pr\u00f3ximos 30 dias.`
        },
        storeDistribution: {
            finalidade: 'Distribui o gasto total entre os estabelecimentos com maior participa\u00e7\u00e3o no per\u00edodo.',
            comportamento: topStore
                ? `${topStore.name} lidera com ${formatCurrency(topStore.value)} e concentra ${formatPercent(topStoreShare)} do total observado.`
                : 'N\u00e3o h\u00e1 dados suficientes para destacar a distribui\u00e7\u00e3o por loja.',
            tendencia: topThreeStoreShare > 0
                ? `Se o padr\u00e3o persistir, as tr\u00eas principais lojas devem continuar respondendo por ${formatPercent(topThreeStoreShare)} do gasto consolidado.`
                : 'Sem base suficiente, a concentra\u00e7\u00e3o por loja permanece inconclusiva.'
        },
        storeComparison: {
            finalidade: 'Compara diretamente as lojas de maior peso financeiro no intervalo filtrado.',
            comportamento: topStore && secondStore
                ? `A diferen\u00e7a entre ${topStore.name} e ${secondStore.name} \u00e9 de ${formatCurrency(topStore.value - secondStore.value)}, o que evidencia a dist\u00e2ncia entre a primeira e a segunda posi\u00e7\u00e3es.`
                : topStore
                    ? `${topStore.name} aparece como a principal refer\u00eancia de gasto no per\u00edodo.`
                    : 'N\u00e3o h\u00e1 base suficiente para comparar estabelecimentos.',
            tendencia: topStore
                ? `A hierarquia atual sugere ${topStoreShare >= 35 ? 'lideran\u00e7a consolidada da principal loja.' : 'disputa relativamente equilibrada entre as lojas mais frequentes.'}`
                : 'Sem dados suficientes, a tend\u00eancia entre lojas permanece indefinida.'
        },
        storeTimeline: {
            finalidade: 'Cruza os gastos mensais com as principais lojas para mostrar sazonalidade e revezamento de relev\u00e2ncia.',
            comportamento: peakMonth
                ? `O m\u00eas de maior volume foi ${peakMonth.month}, com ${formatCurrency(peakMonth.total)}. ${peakMonthTopStore?.label ? `${peakMonthTopStore.label} liderou esse movimento.` : ''}`.trim()
                : 'Ainda n\u00e3o h\u00e1 s\u00e9rie suficiente para comparar meses e lojas.',
            tendencia: lastMonth
                ? `O m\u00eas mais recente (${lastMonth.month}) encerrou com ${formatCurrency(lastMonth.total)}, ${lastMonth.total >= averageMonthValue ? 'acima' : 'abaixo'} da m\u00e9dia mensal do per\u00edodo.`
                : 'Sem mapeamento mensal suficiente, a tend\u00eancia sazonal n\u00e3o pode ser confirmada.'
        },
        accumulated: {
            finalidade: 'Acumula o gasto ao longo do tempo para mostrar o ritmo de forma\u00e7\u00e3o do total desembolsado.',
            comportamento: totalAccumulated > 0
                ? `A curva acumulada encerra o per\u00edodo em ${formatCurrency(totalAccumulated)}. Na metade da janela analisada, o acumulado estava em ${formatCurrency(accumulatedMidPoint)}.`
                : 'Ainda n\u00e3o h\u00e1 dados suficientes para compor a curva acumulada.',
            tendencia: totalAccumulated > 0
                ? `O trecho final da curva aponta ${reportData.temporalProjection.direction} do gasto agregado, com forma\u00e7\u00e3o acelerada quando o consumo recente supera a m\u00e9dia anterior.`
                : 'Sem acumulado consistente, n\u00e3o h\u00e1 tend\u00eancia observ\u00e1vel.'
        },
        pareto: {
            finalidade: 'Ordena os produtos de maior impacto financeiro e mostra a concentra\u00e7\u00e3o acumulada do gasto nos 10 itens principais.',
            comportamento: topParetoItem
                ? `${topParetoItem.name} ocupa a primeira posi\u00e7\u00e3o individual, e os tr\u00eas itens mais relevantes somam ${formatPercent(paretoTopThreeShare)} do recorte exibido.`
                : 'Ainda n\u00e3o h\u00e1 dados suficientes para compor o Pareto dos produtos.',
            tendencia: reportData.paretoData.length
                ? `Os cinco itens mais importantes acumulam ${formatPercent(paretoTopFiveShare)}, indicando ${paretoTopFiveShare >= 75 ? 'alta concentra\u00e7\u00e3o em poucos produtos.' : 'distribui\u00e7\u00e3o relativamente mais equilibrada entre os itens.'}`
                : 'Sem massa cr\u00edtica de produtos, a tend\u00eancia de concentra\u00e7\u00e3o permanece inconclusiva.'
        },
        treemap: {
            finalidade: 'Mostra o peso relativo de cada categoria em um mapa de \u00e1reas, facilitando a leitura visual de domin\u00e2ncia.',
            comportamento: topCategory
                ? `${topCategory.name} ocupa a maior \u00e1rea relativa e responde por ${formatPercent(topCategory.percentage)} do total analisado.`
                : 'Ainda n\u00e3o h\u00e1 categorias suficientes para compor o mapa.',
            tendencia: topThreeCategoryShare > 0
                ? `As tr\u00eas maiores categorias concentram ${formatPercent(topThreeCategoryShare)} do gasto, o que indica ${topThreeCategoryShare >= 70 ? 'forte concentra\u00e7\u00e3o or\u00e7ament\u00e1ria.' : 'uma distribui\u00e7\u00e3o mais difusa entre categorias.'}`
                : 'Sem representatividade suficiente, a tend\u00eancia por categoria n\u00e3o pode ser medida.'
        },
        categories: {
            finalidade: 'Compara a participa\u00e7\u00e3o percentual das categorias no gasto total.',
            comportamento: topCategory
                ? `${topCategory.name} lidera com ${formatPercent(topCategory.percentage)} do total. As tr\u00eas maiores categorias concentram ${formatPercent(topThreeCategoryShare)} do gasto.`
                : 'Sem dados suficientes para classificar a participa\u00e7\u00e3o das categorias.',
            tendencia: topCategory
                ? `Mantido o padr\u00e3o atual, o or\u00e7amento tende a permanecer ${ESSENTIAL_CATEGORIES.has(topCategory.name) ? 'mais ancorado em despesas essenciais.' : 'mais flex\u00edvel entre categorias discricion\u00e1rias.'}`
                : 'Sem base suficiente, a tend\u00eancia por categoria permanece inconclusiva.'
        },
        recurrentProducts: {
            finalidade: 'Destaca os 5 produtos com maior frequ\u00eancia nos cupons.',
            comportamento: reportData.topRecurringProducts.length
                ? `O produto mais frequente do per\u00edodo \u00e9 ${reportData.topRecurringProducts[0].name}, que aparece em ${reportData.topRecurringProducts[0].recurrenceCount} cupons com um impacto financeiro total de ${formatCurrency(reportData.topRecurringProducts[0].totalValue)}.`
                : 'N\u00e3o h\u00e1 registros suficientes para destacar frequ\u00eancia de produtos.',
            tendencia: reportData.topRecurringProducts.length > 0
                ? 'Itens essenciais tendem a liderar esta lista dada \u00e0 natureza recorrente de reposi\u00e7\u00e3o.'
                : 'Tend\u00eancia indefinida pela falta de base comparativa.'
        },
        priceHistogram: {
            finalidade: 'Agrupa os itens por faixa de pre\u00e7o unit\u00e1rio para revelar a concentra\u00e7\u00e3o da cesta.',
            comportamento: dominantPriceBucket
                ? `A faixa unit\u00e1ria mais recorrente \u00e9 ${dominantPriceBucket.name}, com ${dominantPriceBucket.count} itens registrados.`
                : 'Ainda n\u00e3o h\u00e1 itens suficientes para caracterizar a distribui\u00e7\u00e3o de pre\u00e7os.',
            tendencia: totalPriceItems > 0
                ? `As faixas mais baixas representam ${formatPercent(lowPriceShare)} dos itens do histograma, indicando ${lowPriceShare >= 50 ? 'predom\u00ednio de compras de menor valor unit\u00e1rio.' : 'maior mistura entre itens baratos e itens de valor mais elevado.'}`
                : 'Sem volume suficiente, a tend\u00eancia por faixa de pre\u00e7o permanece indefinida.'
        },
        composition: {
            finalidade: 'Mostra como o or\u00e7amento se distribui entre categorias e destaca os principais centros de custo.',
            comportamento: topCategory
                ? `${topCategory.name} lidera a composi\u00e7\u00e3o com ${formatPercent(topCategory.percentage)} do total.`
                : 'Sem dados suficientes para caracterizar a composi\u00e7\u00e3o por categorias.',
            tendencia: topCategory
                ? `Com o padr\u00e3o atual, a cesta tende a permanecer ${ESSENTIAL_CATEGORIES.has(topCategory.name) ? 'centrada em necessidades essenciais.' : 'mais distribu\u00edda entre despesas n\u00e3o essenciais.'}`
                : 'Sem base suficiente, a tend\u00eancia da composi\u00e7\u00e3o permanece inconclusiva.'
        },
        flow: {
            finalidade: 'Mostra a jornada do dinheiro dos estabelecimentos de origem at\u00e9 as categorias de destino.',
            ...
            tendencia: topLink
                ? 'Se o padr\u00e3o persistir, as mesmas conex\u00f5es entre lojas e categorias devem continuar liderando a aloca\u00e7\u00e3o do gasto.'
                : 'Sem volume suficiente, a tend\u00eancia de fluxo permanece inconclusiva.'
        },
        heatmap: {
            finalidade: 'Compara a incid\u00eancia de compras por dia da semana e ajuda a identificar recorr\u00eancia operacional.',
            comportamento: dominantDay
                ? `${dominantDay.name} \u00e9 o dia com maior movimento, com ${dominantDay.value} compras registradas no per\u00edodo.`
                : 'Ainda n\u00e3o h\u00e1 compras suficientes para caracterizar a semana.',
            tendencia: sum(reportData.heatmapData.map((item) => item.value)) > 0
                ? `As compras de fim de semana representam ${formatPercent(weekendShare)} do total, o que sugere ${weekendShare >= 35 ? 'peso relevante do consumo concentrado aos s\u00e1bados e domingos.' : 'maior distribui\u00e7\u00e3o das compras ao longo da semana.'}`
                : 'Sem recorr\u00eancia suficiente, a tend\u00eancia semanal ainda \u00e9 inconclusiva.'
        },
        ticketHistogram: {
            finalidade: 'Agrupa os cupons por faixa de valor total para indicar o padr\u00e3o de tamanho das compras.',
            comportamento: dominantTicketBucket
                ? `A faixa de cupom mais recorrente \u00e9 ${dominantTicketBucket.label}, com ${dominantTicketBucket.count} ocorr\u00eancias.`
                : 'Ainda n\u00e3o h\u00e1 cupons suficientes para caracterizar o tamanho das compras.',
            tendencia: totalReceiptsCount > 0
                ? `Cupons de menor valor representam ${formatPercent(lowTicketShare)} das compras, enquanto os cupons mais altos representam ${formatPercent(highTicketShare)}.`
                : 'Sem volume suficiente, a tend\u00eancia por faixa de cupom permanece indefinida.'
        },
        inflation: {
            finalidade: 'Compara o \u00faltimo pre\u00e7o com o pre\u00e7o anterior dos produtos compar\u00e1veis para medir infla\u00e7\u00e3o pessoal, impacto financeiro e press\u00e3o por categoria.',
            comportamento: reportData.inflation.comparableProductsCount > 0
                ? `${reportData.inflation.comparableProductsCount} produtos compar\u00e1veis sustentam um \u00edndice geral de ${formatSignedRatioPercent(reportData.inflation.personalInflationRate)} e impacto acumulado de ${formatCurrency(reportData.inflation.totalInflationImpact)}.`
                : 'Ainda n\u00e3o h\u00e1 produtos com pelo menos dois pre\u00e7os em datas diferentes dentro do per\u00edodo analisado.',
            tendencia: inflationTopProduct || inflationTopCategory
                ? `A principal press\u00e3o atual vem de ${inflationTopProduct ? `${inflationTopProduct.name} (${formatSignedRatioPercent(inflationTopProduct.inflationRate)})` : 'um produto compar\u00e1vel'}${inflationTopCategory ? ` e da categoria ${inflationTopCategory.name} (${formatSignedRatioPercent(inflationTopCategory.inflationRate)}).` : '.'}`
                : 'Sem tend\u00eancias suficientes, n\u00e3o h\u00e1 produto ou categoria inflacion\u00e1ria em destaque.'
        }
    };
};
...
export const generateConsumptionAnalysisPdf = async ({
    ...
    const chartPages = [
        {
            title: 'Evolu\u00e7\u00e3o de Pre\u00e7o por Produto',
            canvas: drawLineAreaChart({
                title: 'Evolu\u00e7\u00e3o de Pre\u00e7o por Produto',
                subtitle: reportData.selectedProductLabels.length
                    ? `Sele\u00e7\u00e3o atual: ${joinLabels(reportData.selectedProductLabels)}`
                    : 'Selecione produtos no dashboard para compor esta s\u00e9rie',
                data: reportData.productEvolutionData,
                valueKey: 'price',
                labelKey: 'date',
                lineColor: COLORS.navy,
                fillColor: 'rgba(47, 93, 140, 0.12)',
                yFormatter: formatCurrency,
                xFormatter: formatAxisDate,
                emptyMessage: 'Nenhum produto selecionado ou sem hist\u00f3rico suficiente',
                startAtZero: false,
                drawArea: false
            }),
            insights: insights.productEvolution
        },
        {
