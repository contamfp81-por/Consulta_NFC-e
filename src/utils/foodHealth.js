...
const calculateConsistencyFactor = (series = [], index, windowSize = MONTH_WINDOW) => {
...
export const construirIndiceSaudeAlimentar = ({
    ...
    foodClassificationOverrides = [],
    referenceDate = new Date(),
    windowSize = MONTH_WINDOW
} = {}) => {
    const safeReferenceDate = toLocalDate(referenceDate) || new Date();
    const referenceMonthKey = getMonthKey(safeReferenceDate);
    const monthKeys = buildMonthKeys(safeReferenceDate, windowSize);
    ...
    const adjustedSeries = baseSeries.map((month, index, series) => {
        ...
        const { factor: consistencyFactor, volatility } = calculateConsistencyFactor(series, index, windowSize);
