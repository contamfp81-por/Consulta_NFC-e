...
import { ForecastPrecisionView } from './components/ForecastPrecisionModule';
...
function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'home':
        return <Dashboard onOpenForecastPrecision={() => setActiveTab('forecast-precision')} />;
      case 'forecast-precision':
        return <ForecastPrecisionView onBack={() => setActiveTab('home')} />;
      case 'history':
        return <HistoryView />;
      case 'scan':
  ...
  return (
    <div className={`premium-container ${(activeTab === 'home' || activeTab === 'forecast-precision') ? 'premium-container-dashboard' : ''}`}>
      {/* Header */}
      ...
      {activeTab !== 'scan' && activeTab !== 'home' && activeTab !== 'forecast-precision' && (
        <MotionButton
          whileTap={{ scale: 0.9 }}
      ...
      <nav className="nav-bar">
        <button
          className={`nav-item ${(activeTab === 'home' || activeTab === 'forecast-precision') ? 'active' : ''}`}
